...
BATTLE_BULLETS = (
    ...
    "round tests, compare both tools concretely, declare the round winner; 2 or 3 FLOWING "
    "sentences that connect naturally, 28-36 words - long connected passages read with "
    "momentum, short fragments sound robotic); final segment = the verdict spoken naturally plus ONE quick "
...
RANKING_BULLETS = (
    ...
    "segment per tool in countdown order 5 to 1: name the tool, give the concrete reason it "
    "earns the spot, and add one vivid detail or comparison; 2 or 3 FLOWING sentences that "
    "connect naturally, 26-34 words (long connected passages read with momentum; short "
    "fragments make the voice restart its energy and sound robotic); build anticipation "
    "toward number 1. Final segment = crown number 1 in one line, then the cta question "
    "inviting a comment; max 25 words total; no follow/subscribe nudge. Spoken order MUST match the "
...
def generate_ranking_brief(topic, hook_style):
    user = (
        ...
        "Return a single JSON object with these keys:\n"
        "- slug: kebab-case, 3-6 words, no dates\n"
        "- title: a clickable YouTube title, <=70 chars. When the topic is phrased first-person "
        "('I Ranked/Tested N ...'), KEEP that phrasing as the title — it is the channel's "
        "best-performing pattern. Otherwise list-style with curiosity or money framing "
        "(e.g. 'Top 5 Free AI Tools That Feel Illegal to Use', '5 Free Alternatives "
        "to Jasper That Actually Work'), honest, no clickbait lies\n"
        "- hook: the spoken opening line (<=12 words). It must be a scroll-stopping "
        f"pattern-interrupt of this exact style: {HOOK_STYLES[hook_style]}. "
...
def replenish_rankings(topics, want=12):
    try:
        ...
        user = (
            ...
            "channel whose format is 'Top 5 AI tools' rankings. THE CHANNEL'S OWN DATA SAYS "
            "(2026-07-16): its one breakout video ('I Ranked 50 AI Tools for YouTube, Only 5 "
            "Are Worth It', 214 views) beat every sibling (0-35 views, same production) by "
            "20-50x, and it was the ONLY video that (a) targeted CREATORS — an audience "
            "natively on YouTube (YouTubers, Shorts/TikTok makers, streamers, podcasters, "
            "faceless channels) — and (b) used first-person effort-plus-survivor framing "
            "('I ranked/tested N, only 5 are worth it / survived'). So: make roughly half "
            "the ideas creator-audience topics in that exact first-person pattern. Remaining "
            "proven angles: free tools that beat paid ones, free alternatives to a famous "
            "paid tool, best tools for one concrete job, tier-list style 'ranked worst to "
            "best'. Use curiosity and money framing ('feel illegal to use', 'tier list', "
            "'free alternatives to', 'better than ChatGPT'), never neutral catalogue "
            "phrasing. Every idea must name a concrete theme, not a vague 'best AI tools'. "
            "Prefer themes where some of "
